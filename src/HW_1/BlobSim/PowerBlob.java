...
/**
 * A type of blob which will update each term within the blob
 * by putting it to the power of the number of cycles the blob has gone through.
 * 
 * This blob has a built in function which prevents it from storing numbers greater
 * than 10 to the power of the user's choice (default set to 7).
 * @author Manoli Tramountanas
 *
 */
public class PowerBlob extends Blob {

	int numCycles = 0;
	int maxMagnitude = 7;
	
	/**
	 * Constructs a new PowerBlob
	 * for a default maximum magnitude
	 * of one million.
	 */
	public PowerBlob()
	{
		super();
		resetCycles();
		setMaximumMagnitude(7);
	}
	
	/**
	 * Constructs a new PowerBlob
	 * for a maximum magnitude of
	 * 10 to a given power.
	 * @param magnitudePower (int)
	 */
	public PowerBlob(int magnitudePower)
	{
		super();
		resetCycles();
		setMaximumMagnitude(magnitudePower);
	}
	
	/**
	 * Replaces each term with its value
	 * to the power of the number of cycles
	 * the blob has run since the last reset
	 * or since being initialized.
	 */
	@Override
	public void cycle()
	{
		...
		for (int r = 0; r < grid.length; r++)
		{
			...
			{
				grid[r][c] = (int) Math.pow(grid[r][c], numCycles);
			}
		}
	...
	/**
	 * Resets the number of cycles run by the PowerBlob.
	 */
	...
	/**
	 * Sets the maximum magnitude of the terms of the PowerBlob.
	 * @param powerOfTen that the maximum magnitude should be set to.
	 */
	public void setMaximumMagnitude(int powerOfTen)
	{
		maxMagnitude = powerOfTen;
	}
	
	/**
	 * Ensures that none of the terms within the PowerBlob
	 * are above the maximum magnitude.
	 * If a term is above the maximum magnitude,
	 * set it to the remainder of the term
	...
	private void checkTerms()
	{
		for (int r = 0; r < grid.length; r++)
		{
			for (int c = 0; c < grid[r].length; c++)
			{
				grid[r][c] %= Math.pow(10, maxMagnitude);
			}
		}
	}
}
